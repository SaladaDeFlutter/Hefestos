...
async function upsertConfig(guildId, guildName, provider, model, apiKey, discordToken) {
  const { rows } = await pool.query(
    `INSERT INTO bot_configs (guild_id, guild_name, provider, model, api_key, discord_token)
     VALUES ($1, $2, $3, $4, $5, $6)
     ON CONFLICT (guild_id) DO UPDATE SET
       guild_name = $2, provider = $3, model = $4, api_key = $5, discord_token = $6,
       updated_at = NOW()
     RETURNING *`,
    [guildId, guildName, provider, model, apiKey, discordToken]
  );
  return rows[0];
}

async function updateToken(guildId, discordToken) {
  const { rows } = await pool.query(
    `UPDATE bot_configs SET discord_token = $2, updated_at = NOW() WHERE guild_id = $1 RETURNING *`,
    [guildId, discordToken]
  );
  return rows[0];
}
...
module.exports = { migrate, getConfig, upsertConfig, updateToken, getSession, saveSession, listConfigs, deleteConfig };
