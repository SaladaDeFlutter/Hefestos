...
function start(botRow) {
  ...
  client.on('ready', async () => {
    try {
      ...
      if (!botRow.session_id) {
        ...
        console.log(`[${botRow.name}] Session: ${session.id}`);
      }
    } catch (err) {
      console.error(`[${botRow.name}] Init error:`, err.message);
    }
  });
  ...
  client.on('messageCreate', async (msg) => {
    try {
      ...
      console.error(`[${botRow.name}] Message error:`, err.message);
    }
  });

  client.login(botRow.discord_token).then(() => {
    console.log(`[${botRow.name}] Logged in`);
  }).catch(err => {
...
async function processQueue(botRow) {
  ...
  try {
    ...
    console.error(`[${botRow.name}] Process error:`, err.message);
