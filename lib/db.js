...
async function deleteBotSession(botId) {
  ...
  if (rows[0]?.session_id) {
    try {
      await fetch(`http://127.0.0.1:10001/session/${rows[0].session_id}`, { method: 'DELETE' });
    } catch (e) {}
  }
...
module.exports = { migrate, setBotSession, deleteBotSession, listBots, createBot, updateBot, deleteBot, getBot, getSession, saveSession, deleteSession };
