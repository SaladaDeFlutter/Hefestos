...
function start(botRow) {
  ...
  client.on('messageCreate', async (msg) => {
    ...
    const isGeneralChat = !isDM && isGeneralChannel(msg.channel) && Math.random() < 0.3;
    if (!isDM && !isMention && !isGeneralChat) return;
...
function isGeneralChannel(channel) {
  ...
  return ['geral', 'general', 'chat', 'bate-papo', 'conversa', 'off-topic', 'offtopic', 'random', 'lounge'].some(k => name.includes(k));
}
